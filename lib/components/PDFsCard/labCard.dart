...
class labWorkSet extends StatefulWidget {
  final snap;
  ...
  @override
  State<labWorkSet> createState() => _labWorkSetState();
}

class _labWorkSetState extends State<labWorkSet> {
  ...
  likeSystem() {
    if (snap["liked user"].contains(username)) {
      PdfData.updateUnlike(username!, "Lab work", snap["Pid"]);
      likes = Icons.favorite_border;
    } else {
      PdfData.updateLike(username!, "Lab work", snap["Pid"]);
  ...
  bookmarkSystem() {
    ...
    PdfData.extraUpdateBookmark(snap, username!, "Lab work");
  }
